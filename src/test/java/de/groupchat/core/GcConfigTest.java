...
class GcConfigTest {
    ...
    @Test void generatesDocumentedDefaults() throws Exception {
        ...
        assertTrue(Files.readString(file).contains("chat_log_retention_hours = -1"));
    }
    ...
    @Test void invalidConfigFailsWithoutOverwritingIt() throws Exception {
        ...
        for (String invalid : new String[]{"max_owned_groups = 0", "invite_cooldown_seconds = -1", "invite_expiry_days = 0",
                ...
                "chat_log_retention_hours = 0", "chat_log_retention_hours = -2", "chat_log_retention_hours = 87601",
    ...
    @Test void acceptsDisabledAndPositiveRetentionWithoutChangingExistingSettings() throws Exception {
        Path file = directory.resolve("groupchat.toml");
        for (long hours : new long[]{-1, 1, 24, 87_600}) {
            String text = "# Keep this setting\nchat_log_retention_hours = " + hours + "\n";
            Files.writeString(file, text);
            assertEquals(hours, GcConfig.load(file).chatLogRetentionHours());
            assertEquals(text, Files.readString(file));
        }
    }
    @Test void zeroRetentionExplainsHowToDisableLogging() throws Exception {
        Path file = directory.resolve("groupchat.toml");
        Files.writeString(file, "chat_log_retention_hours = 0\n");
        var error = assertThrows(IllegalArgumentException.class, () -> GcConfig.load(file));
        assertTrue(error.getMessage().contains("-1 to disable logging"));
        assertEquals("chat_log_retention_hours = 0\n", Files.readString(file));
    }
    ...
    @Test void migratesOldConfigurationWithCustomValuesAndComments() throws Exception {
        ...
        assertEquals(new GcConfig(90, 5, 14, -1), GcConfig.loadFromConfigDirectory(directory));
        ...
        assertTrue(Files.readString(migrated).contains("chat_log_retention_hours = -1"));
    }
    ...
    @Test void upgradesRootSettingWithoutAccidentallyAddingItToAnExistingTomlTable() throws Exception {
        ...
        assertEquals(-1, GcConfig.load(file).chatLogRetentionHours());
        String upgraded = Files.readString(file);
        assertEquals(new GcConfig(60, 4, 7, -1), GcConfig.load(file));
